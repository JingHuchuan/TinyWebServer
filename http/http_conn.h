#ifndef HTTPCONNECTION_H
#define HTTPCONNECTION_H

#include <unistd.h>
#include <signal.h>
#include <sys/types.h>
#include <sys/epoll.h>
#include <fcntl.h>
#include <sys/socket.h>
#include <netinet/in.h>
#include <arpa/inet.h>
#include <assert.h>
#include <sys/stat.h>
#include <string.h>
#include <pthread.h>
#include <stdio.h>
#include <stdlib.h>
#include <sys/mman.h>
#include <stdarg.h>
#include <sys/wait.h>
#include <errno.h>
#include <sys/uio.h>
#include <string>
#include <map>

#include "../lock/locker.h"
#include "../CGImysql/sql_connection_pool.h"
#include "../timer/lst_timer.h"
#include "../log/log.h"

using namespace std;

class http_conn {
public:
    static const int FILENAME_LEN = 200;                        // 设置读取文件的名称m_real_file大小
    static const int READ_BUFFER_SIZE = 2048;                   // 设置读缓冲区m_read_buf大小
    static const int WRITE_BUFFER_SIZE = 1024;                  // 设置写缓冲区m_write_buf大小

    // HTTP请求方法，本项目只用到GET和POST
    enum METHOD { GET = 0, POST, HEAD, PUT, DELETE, TRACE, OPTIONS, CONNECT, PATH };

    /*
        解析客户请求时，主状态机所处的状态
        CHECK_STATE_REQUESTLINE   :   当前正在分析请求行
        CHECK_STATE_HEADER        :   当前正在分析头部字段
        CHECK_STATE_CONTENT       :   当前正在解析请求体
    */
    enum CHECK_STATE { CHECK_STATE_REQUESTLINE = 0, CHECK_STATE_HEADER, CHECK_STATE_CONTENT };

    /*
        从状态机的三种可能状态
        LINE_OK    :    读取到一个完整的行
        LINE_BAD   :    行出错
        LINE_OPEN  :    行数据尚且不完整
    */
    enum LINE_STATUS { LINE_OK = 0, LINE_BAD, LINE_OPEN };

    /*
        服务器处理HTTP请求的可能结果，报文解析的结果
        NO_REQUEST        :  请求不完整，需要继续读取客户数据
        GET_REQUEST       :  表示获得了一个完整的客户请求
        BAD_REQUEST       :  表示客户请求语法错误
        NO_RESOURCE       :  表服务器没有资源
        FORBIDDEN_REQUEST :  表示客户对资源没有足够的访问权限
        FILE_REQUEST      :  文件请求，获取文件成功
        INTERNAL_ERROR    :  表示服务器内部错误
        CLOSED_CONNECTION :  表示客户端已经关闭连接了
    */
    enum HTTP_CODE { NO_REQUEST, GET_REQUEST, BAD_REQUEST, 
                   NO_RESOURCE, FORBIDDEN_REQUEST, FILE_REQUEST,
                   INTERNAL_ERROR, CLOSED_CONNECTION };

    static int m_epollfd;                                       // 所有的socket上的事件都被注册到同一个epoll对象，所以需要初始化为static变量
    static int m_user_count;                                    // 统计用户的数量，也需要初始化为static变量
    MYSQL *mysql;                                               // 数据库对象指针
    int m_state;                                                // 读为0, 写为1
    int timer_flag;                                             // 下面两个标志位是Reactor模式专属，保持主线程和子线程的同步，0未处理完这次的请求，1已处理完
    int improv;                                                 // 0不需要移除timer，1需要移除timer(断开连接)

public:
    http_conn();
    ~http_conn();

    void init(int sockfd, const sockaddr_in& addr, char*, int,  // 初始化套接字地址，函数内部会调用私有方法init()
    int, string user, string passwd, string sqlname);           
    void close_conn(bool real_close = true);                    // 关闭连接
    void process();                                             // 处理客户端请求
    bool read_once();                                           // 一次性读取数据
    bool write();                                               // 响应报文写入函数
    sockaddr_in *get_address() {                                // 获取socket地址
        return &m_address;
    }
    void initmysql_result(connection_pool *connPool);           // 同步线程初始化数据库读取表

private:
    int m_sockfd;                                               // 该HTTP连接的socket
    sockaddr_in m_address;                                      // 通信的socket地址
    char m_read_buf[READ_BUFFER_SIZE];                          // 读缓冲区
    long m_read_idx;                                            // 标识读缓冲区已经读入的客户数据的最后一个字节的下一个位置
    long m_checked_idx;                                         // 当前正在分析的字符在缓冲区中的位置
    int m_start_line;                                           // 当前正在解析的行的起始位置
    char m_write_buf[WRITE_BUFFER_SIZE];                        // 写缓冲区
    int m_write_idx;                                            // 写缓冲区中待发送的字节数
    CHECK_STATE m_check_state;                                  // 主状态机当前所处的状态
    METHOD m_method;                                            // 请求方法
    char m_real_file[FILENAME_LEN];                             // 客户请求的目标文件的完整路径，其内容等于doc_root + m_url, doc_root是网站根目录
    char* m_url;                                                // 请求目标文件的文件名
    char* m_version;                                            // 协议版本，目前只支持HTTP1.1
    char* m_host;                                               // 主机名
    bool m_linger;                                              // HTTP请求是否要保持连接Connection: keep-alive
    long m_content_length;                                      // HTTP请求的消息体的长度
    char * m_file_address;                                      // 客户请求的目标文件被mmap到内存中的起始位置
    struct stat m_file_stat;                                    // 目标文件的状态, 通过它我们可以判断文件是否存在、是否为目录、是否可读，并获取文件大小等信息
    struct iovec m_iv[2];                                       // 我们将采用writev来执行写操作，所以定义下面两个成员
    int m_iv_count;                                             // 被写内存块的数量
    int cgi;                                                    // 是否启用的POST
    char *m_string;                                             // 存储请求体数据，POST请求存放的就是用户名和密码
    int bytes_to_send;                                          // 剩余发送字节数
    int bytes_have_send;                                        // 已经发送的数据
    char *doc_root;                                             // 网站的根目录
    map<string, string> m_users;                                // 用户名-密码映射
    int m_TRIGMode;                                             // 触发方式
    int m_close_log;                                            // 日志开关
    char sql_user[100];                                         // 用户名
    char sql_passwd[100];                                       // 密码
    char sql_name[100];                                         // 数据库名

private:
    void init();                                                // 初始化连接的其他信息
    HTTP_CODE process_read();                                   // 从m_read_buf读取，并处理请求报文
    bool process_write(HTTP_CODE ret);                          // 向m_write_buf写入响应报文数据

    // 下面的函数被process_read调用以分析HTTP请求
    HTTP_CODE parse_request_line(char* text);                   // 解析请求首行
    HTTP_CODE parse_headers(char* text);                        // 解析头部字段
    HTTP_CODE parse_content(char* text);                        // 解析请求体
    LINE_STATUS parse_line();                                   // 从状态机，用于解析出一行的内容，然后给相应的解析函数解析
    HTTP_CODE do_request();                                     // 得到一行之后做具体的处理
    char* get_line() { return m_read_buf + m_start_line; }      // 从m_read_buf中获取一行

    // 下面的函数被process_write调用以填充HTTP应答
    void unmap();                                               // 内存映射
    bool add_response(const char* format, ...);                 // 往写缓冲中写入代发送数据
    bool add_content(const char* content);                      // 添加响应状态行
    bool add_status_line(int status, const char* title);        // 添加状态行（对应请求行）
    bool add_headers(int content_length);                       // 添加头部信息
    bool add_content_length(int content_length);                // 添加消息体长度
    bool add_content_type();                                    // 添加相应数据类型
    bool add_linger();                                          // 添加HTTP是否保持连接
    bool add_blank_line();                                      // 添加空行
};
#endif
